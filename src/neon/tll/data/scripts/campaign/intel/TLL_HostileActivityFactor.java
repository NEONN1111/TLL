...
import neon.tll.data.scripts.campaign.ids.TLL_IDs;
...
import java.util.List;
    ...
    public class TLL_HostileActivityFactor extends BaseHostileActivityFactor implements FleetGroupIntel.FGIEventListener {
        public TLL_HostileActivityFactor(HostileActivityEventIntel intel) {
            super(intel);
        }
...
//	}
        ...
        public static String RAID_KEY = "$TLLraid_ref";

        public static final String DEFEATED_TLL_RAID = "$defeatedTLLRaid";

        public static String ACCEPTED_TERMS = "acceptedTLLTerms";

        public static boolean getDefeatedTllRaid() {
            //if (true) return true;
            return Global.getSector().getPlayerMemoryWithoutUpdate().getBoolean(DEFEATED_TLL_RAID);
        }

        public static void setDefeatedTllRaid(boolean value) {
            Global.getSector().getPlayerMemoryWithoutUpdate().set(DEFEATED_TLL_RAID, value);
        }

        public static boolean getAcceptedTLLTerms() {
            //if (true) return true;
            return Global.getSector().getPlayerMemoryWithoutUpdate().getBoolean(ACCEPTED_TERMS);
        }

        public static void setAcceptedTLLTerms(boolean value) {
            Global.getSector().getPlayerMemoryWithoutUpdate().set(ACCEPTED_TERMS, value);
        }
        ...
        public String getDesc(BaseEventIntel intel) {
            return "Tellasian activity";
        }

        public String getNameForThreatList(boolean first) {
            if (first) return "Tellasian League";
            return "Tellasian League";
        }
        ...
        public Color getDescColor(BaseEventIntel intel) {
            ...
            return Global.getSector().getFaction(TLL_IDs.tll).getBaseUIColor();
        }
        ...
        public TooltipMakerAPI.TooltipCreator getMainRowTooltip(BaseEventIntel intel) {
            return new BaseFactorTooltip() {
                public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
                    float opad = 10f;
                    tooltip.addPara("Tellusians are silly goofy.", 0f);

                    tooltip.addPara("However, you have %s, which is enough dissuade most Tellusians from attacking your interests.",
                            opad, Misc.getPositiveHighlightColor(), "dope ass aura and clean vibes");
        ...
        public boolean shouldShow(BaseEventIntel intel) {
            return getProgress(intel) > 0 || getAcceptedTLLTerms();
        }
...
//		int minDiff = Math.max(0, size - 2);
        ...
        public void addBulletPointForEvent(HostileActivityEventIntel intel, BaseEventIntel.EventStageData stage, TooltipMakerAPI info,
            ...
            info.addPara("Rumors of Tellusian raid", tc, initPad);
...
//		info.addPara("Rumors of pirate raid", initPad, tc, c, "pirate raid");
        ...
        public void addBulletPointForEventReset(HostileActivityEventIntel intel, BaseEventIntel.EventStageData stage, TooltipMakerAPI info,
                                                IntelInfoPlugin.ListInfoMode mode, boolean isUpdate, Color tc, float initPad) {
            info.addPara("Tellusian raid averted", tc, initPad);
        }
...
//				0f, Misc.getPositiveHighlightColor(), "increased accessibility");
            ...
            info.addPara("There are rumors that a Tellusian raid targeting your colonies "
...
//		}
            ...
            addBorder(info, Global.getSector().getFaction(TLL_IDs.tll).getBaseUIColor());
...
//				info.getWidthSoFar() + small * 2f, Math.max(64f, info.getHeightSoFar() + small));
        ...
        public String getEventStageIcon(HostileActivityEventIntel intel, BaseEventIntel.EventStageData stage) {
            return Global.getSector().getFaction(TLL_IDs.tll).getCrest();
        }
            ...
            if (stage.id == HostileActivityEventIntel.Stage.HA_EVENT || stage.id == HostileActivityEventIntel.Stage.MINOR_EVENT) {
                if (stage.id == HostileActivityEventIntel.Stage.MINOR_EVENT) {
                    return new BaseFactorTooltip() {
                        ...
                        public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
                            float opad = 10f;
                            tooltip.addTitle("Tellusian raid");
                            tooltip.addPara("A Tellusian raid will be launched against one of your "
                                    + "star systems.", opad);
                        }
                    };
                }
                return getDefaultEventTooltip("Tellusian raid", intel, stage);
//			return new BaseFactorTooltip() {
//				@Override
//				public void createTooltip(TooltipMakerAPI tooltip, boolean expanded, Object tooltipParam) {
//					float opad = 10f;
...
//		if (true) return;
        ...
        public StarSystemAPI findRaidTarget(HostileActivityEventIntel intel, BaseEventIntel.EventStageData stage) {
            ...
            for (StarSystemAPI system : Misc.getPlayerSystems(false)) {
                ...
                if (mag < 0.1f && stage.id != HostileActivityEventIntel.Stage.MINOR_EVENT) {
                    //if (mag < 0.2f) {
                    continue;
                }
        ...
        public MarketAPI findRaidSource(HostileActivityEventIntel intel, BaseEventIntel.EventStageData stage, final StarSystemAPI target) {
            ...
            for (MarketAPI market : Global.getSector().getEconomy().getMarketsCopy()) {
                if (TLL_IDs.tll.equals(market.getFaction().getId())) {
        ...
        public static void avertOrAbortRaid() {
            ...
            if (data != null && data.factor instanceof TLL_HostileActivityFactor) {
                intel.resetHA_EVENT();
            }
        }
        ...
        public boolean startRaid(MarketAPI source, StarSystemAPI target, BaseEventIntel.EventStageData stage, Random random) {
            ...
            GenericRaidFGI raid = new TLL_HostileActivityRaidIntel(params);
            if (stage.id == HostileActivityEventIntel.Stage.HA_EVENT) { // don't want piracy respite from the minor raid
