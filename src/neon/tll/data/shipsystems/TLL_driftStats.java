...
public class TLL_driftStats extends BaseShipSystemScript {
    ...
    public void apply(MutableShipStatsAPI stats, String id, State state, float effectLevel) {
        ...
        if(ship!=null){
            ...
            if(state==State.IN && !stats.getMaxSpeed().getPercentMods().containsKey(id)){
                Global.getSoundPlayer().playSound("system_phase_skimmer", 1, 1.75f, ship.getLocation(), ship.getVelocity());
            }
            if(state==State.ACTIVE && !stats.getTimeMult().getPercentMods().containsKey(id)){
                Global.getSoundPlayer().playSound("system_phase_skimmer", 1, 1.5f, ship.getLocation(), ship.getVelocity());
            }
        }
